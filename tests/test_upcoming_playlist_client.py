...
from functions.upcoming_playlist import UpcomingPlaylistClient
...
class TestAddArtist:
    ...
    def test_tracks_added_to_new_playlist(self, artist_id, sp_top_tracks, top_track_uris):
        client = UpcomingPlaylistClient(sp_top_tracks)
    ...
    @pytest.mark.parametrize("artist_id", [ARTIST_NOT_IN_PLAYLIST, ARTIST_WITH_ONE_SONG])
    def test_tracks_added_to_existing_playlist(self, artist_id, sp_top_tracks, top_track_uris):
        client = UpcomingPlaylistClient(sp_top_tracks, PLAYLIST_ID)
        client.add_artist(artist_id)
        sp_top_tracks.playlist_add_items.assert_called_with(
            playlist_id=PLAYLIST_ID,
            items=top_track_uris
